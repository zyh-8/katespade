...
var str = location.href; // console.log(str);

var arr = str.split("?"); //以？分割为数组["http://...html","search="]

var arr1 = arr[1].split("="); //把id=7以=分割为数组["search","袜子"]

var keyval = arr1[1]; //袜子

keyval = decodeURIComponent(keyval); // console.log(keyval);

var filter_title = document.querySelector(".filter_title");
var product = document.querySelector(".product");
var sort = document.querySelector(".sort");
var more_wrap = document.querySelector(".more_wrap");
console.log(product); // 根据关键字，到数据库模糊查询，将查询到的数据渲染到页面
// 如果搜索无结果，显示【搜素结果为0】
// 请求数据
// 根据keyword获取数据
...
pAjax({
  ...
  res = JSON.parse(res);
  console.log(res); // 把数据存储到本地

  localStorage.setItem('goodsList', JSON.stringify(res));
  filter_title.innerHTML = "".concat(keyval, "(").concat(res.length, ")");
  var more = document.querySelector(".more"); // 如果数据小于4条，则移除【查看更多】
  ...
  render(arr);
  arr = []; // 点击【查看更多】加载数据,每次渲染4条

  var page = 1;

  more.onclick = function () {
    var len = res.length - page * 4;
    console.log(len);

    if (len < 4) {
      // 渲染剩下不足4条的数据
      for (var _i = 4 * page; _i < 4 * page + len; _i++) {
        arr.push(res[_i]); // 把数据存储到本地
    ...
    for (var _i2 = 4 * page; _i2 < 4 * (page + 1); _i2++) {
      arr.push(res[_i2]); // 把数据存储到本地

      localStorage.setItem('goodsList', JSON.stringify(arr));
    }

    render(arr);
    more.innerHTML = "\u67E5\u770B\u66F4\u591A(".concat(len, ")");
    arr = [];
    page++;
  };

  console.log(sort.value); // 排序
...
function render(data) {
  ...
  if (!data.length) {
    filter_title.innerHTML = "".concat(keyval, "(0)");
    product.innerHTML = "\n        <div class=\"result\"\n             <p>\u641C\u7D22\u7ED3\u679C\u4E3A\u65E0</p>\n        </div>";
    return;
  }

  data.forEach(function (item, index) {
    str += "\n        <div class=\"product_list\" id=\"".concat(item.goods_id, "\">\n            <div class=\"list_img\"><img src=\"").concat(item.goods_small_logo, "\" alt=\"\" id=\"").concat(item.goods_id, "\"></div>\n            <div class=\"list_title\" id=\"").concat(item.goods_id, "\">").concat(item.goods_name, "</div>\n            <div class=\"list_price\">\uFFE5").concat(item.goods_price, "</div>\n        </div>");
  });
